...
export function ProfileBadge({ profile, current, onFollow, profileLoading }) {
  ...
  return profile ? (
    <div className="bg-white border border-info rounded col-5 mt-4 mb-5 p-5 mx-auto">
      ...
      <p className="font">
        <DisplayCount>{profile.followers_count}</DisplayCount>{" "}
        {profile.followers_count === 1 ? "follower" : "followers"}{" "}
      </p>
      <p className="font">
        <DisplayCount>{profile.following_count}</DisplayCount> following
      </p>
      <p className="font mt-5">{profile.location}</p>
      <p className="font mt-4 mb-4">{profile.bio}</p>
      {profile.username !== current.username && (
        <button className="btn btn-primary font2 mt-4" onClick={handleToggle}>
          {action}
        </button>
      )}
    </div>
