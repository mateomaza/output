  ...
  return (
    <div className="mx-auto mt-2 mb-5 bg-white text-dark w-75 rounded">
      <div className="border border-info rounded">
        <div className="d-flex justify-content-end p-1"></div>
        {isRepost === true && (
          <div className="border-bottom font5 mb-3">
            Repost via <ProfileDisplay profile={post.profile} />
          </div>
        )}
        <div>
          <div className="d-flex mt-3">
            ...
            {isRepost !== true &&
              post.profile.username === current.username && (
        ...
        <div className="btn btn-group mb-3 col-12 d-flex justify-content-end">
          ...
          {isDetail === false && isRepost && (
            <button className="rounded font2" onClick={handlePostLink}>
              View
            </button>
          )}
        </div>
...
export function Repost({ post }) {
  ...
  return post.is_repost === true ? (
    <Post className={" "} {...repostProps} />
  ) : null;
}
...
export function Button({ post, action, onAction }) {
  ...
  const [buttonClass, setButtonClass] = useState(
    post && post.has_liked ? likeClass : unlikeClass
  );
  ...
  if (action === "like" || action === "unlike") {
    ...
    return (
      <>
        {!showConfirmation && (
          <button
            className="btn btn-white btn-sm font2 border-dark rounded ml-1"
        ...
        {showConfirmation && (
          <dialog>
            ...
            <button
              className="btn btn-white btn-sm font2 border-dark rounded mr-1"
              onClick={handleClick}
            >
              Yes
            </button>
            <button
              className="btn btn-white btn-sm font2 border-dark rounded"
...
export function DeleteButton({ post }) {
  ...
  const handleDelete = () => {
    axios
      .post(`http://localhost:8000/api/posts/${post.id}/delete/`, {
        headers: {
  ...
  return (
    <>
      {!showConfirmation && (
        <button
          className="btn btn-danger btn-sm font2 border-dark rounded ml-auto mr-2 mb-5"
          onClick={() => setShowConfirmation(true)}
        >
          X
        </button>
        ...
        <div className="col-12 mx-auto mb-3 p-5" style={{ width: "800px" }}>
          <form onSubmit={handleSubmit}>
            <div className="d-flex mb-3">
              ...
              <textarea
                ref={inputRef}
                className="form-control mx-3 ml-5 font8"
                name="content"
                style={{ height: "150px" }}
              ></textarea>
              <button
                type="submit"
                className="btn btn-primary mx-3 align-self-center font2"
                style={{ height: "60px" }}
              >
                Post
              </button>
...
export function PostDetail({ id }) {
  ...
  return post === null ? null : (
    <div className="bg-dark">
      <Post post={post} className="text-center" />
    </div>
  );
}
